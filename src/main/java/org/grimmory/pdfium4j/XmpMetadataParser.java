 ...
 * Calibre namespaces. Ported from Grimmory's PdfMetadataExtractor patterns with secure XML parsing.
 */
public final class XmpMetadataParser {
  ...
  public static XmpMetadata parse(byte[] xmpBytes) {
    ...
    try {
      ...
      Document doc =
          factory.newDocumentBuilder().parse(new InputSource(new ByteArrayInputStream(xmpBytes)));
  ...
  @SuppressFBWarnings(
      value = "REC_CATCH_EXCEPTION",
      justification =
          "Malformed metadata should degrade to empty metadata instead of failing the document")
  ...
  /**
   * Sanitizes XMP XML strings to handle known vendor corruption patterns. Handles: BOM prefixes,
  ...
  /**
   * Get the first text value from a Dublin Core element. Handles both simple text and RDF
   * Alt/Bag/Seq containers.
   */
  ...
  /**
   * Extract Calibre-specific metadata fields, including series_index from the calibreSI namespace
  ...
  private static List<QualifiedIdentifier> extractXmpIdentifiers(Document doc) {
    List<QualifiedIdentifier> ids = new ArrayList<>();
