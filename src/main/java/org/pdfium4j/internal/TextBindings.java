...
public final class TextBindings {
    ...
    /**
     * Get text in a character range (UTF-16LE output).
     * Returns number of characters written (including null terminator).
     *
     * <p>Parameters: textPage (FPDF_TEXTPAGE handle), startIndex (0-based),
     * count (number of characters to extract), result (UTF-16LE output buffer,
     * must hold (count+1)*2 bytes).
     */
    ...
    /**
     * Get bounded text within a rectangle (UTF-16LE output).
     * Returns number of characters written (including null terminator).
     */
    ...
    /**
     * Get character bounding box by index.
     * Writes left, right, bottom, top to the output pointers (doubles).
    ...
    public static final MethodHandle FPDFText_GetFontSize = downcallCritical("FPDFText_GetFontSize",
            FunctionDescriptor.of(JAVA_DOUBLE, ADDRESS, JAVA_INT));

    /**
     * Get the origin point (baseline start) of a character by index.
    ...
    /**
     * Get URL of a web link (UTF-16LE, double-call pattern).
     * Returns number of characters (including null terminator).
     */
    ...
    /**
     * Get a bounding rectangle for a web link.
     * Writes left, top, right, bottom to output pointers (doubles).
     * Returns 1 on success.
     */
