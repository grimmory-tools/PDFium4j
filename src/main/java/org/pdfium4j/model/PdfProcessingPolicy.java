 ...
 *
 * <p>Performance-critical paths are constrained through explicit memory budgets
 * so FFM allocations and native bitmap operations stay predictable.
 */
public record PdfProcessingPolicy(
    ...
    public static final int DEFAULT_MAX_PARALLEL_THREADS = Math.max(1, Runtime.getRuntime().availableProcessors());
    ...
    public PdfProcessingPolicy withMode(Mode value) {
        return new PdfProcessingPolicy(value, maxDocumentBytes, maxRenderPixels, maxParallelRenderThreads, fileBackedThreshold);
    }

    public PdfProcessingPolicy withMaxDocumentBytes(long value) {
        return new PdfProcessingPolicy(mode, value, maxRenderPixels, maxParallelRenderThreads, fileBackedThreshold);
    }

    public PdfProcessingPolicy withMaxRenderPixels(long value) {
        return new PdfProcessingPolicy(mode, maxDocumentBytes, value, maxParallelRenderThreads, fileBackedThreshold);
    }

    public PdfProcessingPolicy withMaxParallelRenderThreads(int value) {
        return new PdfProcessingPolicy(mode, maxDocumentBytes, maxRenderPixels, value, fileBackedThreshold);
    }

    public PdfProcessingPolicy withFileBackedThreshold(long value) {
        return new PdfProcessingPolicy(mode, maxDocumentBytes, maxRenderPixels, maxParallelRenderThreads, value);
    }
}
